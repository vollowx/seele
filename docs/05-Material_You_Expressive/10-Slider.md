...
```html
<md-slider data-aria-label="Brightness" labeled size="l" stop value="75"></md-slider>
<md-slider
  range
  ticks
  step="10"
...
| `stop`    | Reflected boolean; show stop mark; conflict with `ticks` and `range` |
| `ticks`   | Reflected boolean; show tick marks                                   |
